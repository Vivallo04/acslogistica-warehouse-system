...
export function PrinterConfigurationSection({
  onPrinterSelected
}: PrinterConfigurationSectionProps) {
  ...
  const discoverPrinters = useCallback(async () => {
    ...
    try {
      ...
      // Only show success toast when printers are found
      if (result.jsprintManagerAvailable && (result.localPrinters.length > 0 || result.networkPrinters.length > 0)) {
        toast({
      ...
      // No toasts for JSPrintManager not detected or no printers found - UI shows this info anyway
    } catch (error) {
      // Only show error toast for unexpected errors, not for normal "not found" scenarios
      console.warn('Printer discovery error:', error)
    } finally {
